...
def load_state_dict(
        ...
        load_mode: str = None,
) -> Union[Tuple[Module, Module, Any, Any, Any, Optional[Optimizer], Any], Tuple[
    Module, Any, Any, Any, Optional[Optimizer], Any], Module]:
    # Load model weights
    ...
    if load_mode == "resume":
        ...
        if hasattr(checkpoint, 'best_psnr') and hasattr(checkpoint, 'best_ssim'):
            best_psnr = checkpoint["best_psnr"]
            best_ssim = checkpoint["best_ssim"]
            print(f'best_psnr: {best_psnr}, best_ssim: {best_ssim}')
